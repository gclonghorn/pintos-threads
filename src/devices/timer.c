...
timer_sleep (int64_t ticks)
{
  ...
  current_thread->block_tick = ticks;
  thread_block ();
  intr_set_level (old_level);
}
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ticks++;
  enum intr_level old_level = intr_disable ();
  thread_foreach (thread_block_judge, NULL);
  if (thread_mlfqs)
  {
    /*Each time a timer interrupt occurs, recent_cpu inc 1 for running thread only*/
    ...
    /* recalculated once every fourth clock tick, for every thread*/
    else if (ticks % 4 == 0)
