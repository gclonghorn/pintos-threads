...
/*newly added.Unlike priority and recent_cpu, load_avg is system-wide, not thread-specific.*/
fixed_t load_avg;
...
thread_unblock (struct thread *t)
{
  ...
  /* ready_list sort by priority */
  /*list_push_back(&ready_list,&t->elem);*/
  list_insert_ordered (&ready_list, &t->elem, (list_less_func *) &thread_sort_priority, NULL);
...
thread_yield (void)
{
  ...
  if (cur != idle_thread)
    ...
    list_insert_ordered (&ready_list, &cur->elem, (list_less_func *) &thread_sort_priority, NULL);
...
thread_set_nice (int nice)
{
  struct thread *cur=thread_current ();
  cur->nice = nice;
  /*recalculates the thread's priority based on the new nice---priority = PRI_MAX - (recent_cpu / 4) - (nice * 2),*/
...
void
thread_block_judge (struct thread *t, void *aux UNUSED)
{
  if (t->block_tick > 0)
  {
      t->block_tick--;
...
update_priority_mlfqs (struct thread *t,void *aux)
{
 ...
 recent_cpu  recalculated for every thread*/
void
update_load_avg_and_recent_cpu (void)
{
  /*ready_threads is the number of threads that are either running or ready to run at time of update*/
  size_t ready_threads = list_size (&ready_list);
  if (thread_current ()!=idle_thread)/*idle_thread run when no other thread ready to run*/
    ready_threads++;
  load_avg = FP_ADD (FP_DIV_MIX (FP_MULT_MIX (load_avg, 59), 60), FP_DIV_MIX (FP_CONST (ready_threads), 60));
  thread_foreach(update_recent_cpu,NULL);
}
/*update recent cpu based on new load_avg*/
